...
class CuttingStock(object):
    def __init__(self, customer_count, customer_length, roll_length,name_, **parameters):
        path = 'inputs/given_solutions/solutions.xlsx'
        global df_optiaml_val
        ...
        try:
            self.optimal_val = df_optiaml_val.iloc[df_optiaml_val['Name'] == self.name, "Best LB"].item()
        except:
            pass
        ...
        ## constants
        self.pool_size = parameters['parameters']['action_pool_size']
        self.alpha_obj_weight = parameters['parameters']['alpha_obj_weight']
    ...
    def define_master_problem(self, patterns, vtype='continuous'):
    ...
    def step(self,action, Train = True):
        ...
        if done:
            ...
            reward = self.alpha_obj_weight*(self.objVal_history[-2] - self.objVal_history[-1])/(self.objVal_history[0] ) + self.roll_len * 10
            master_problem = self.define_master_problem(patterns, vtype='integer')
            master_problem.optimize()
            self.objVal_history.append(master_problem.objVal)
