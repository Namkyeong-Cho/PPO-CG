...
class BipartiteGNN(K.Model):
    ...
    def __init__(self, embedding_size=32, cons_num_features=2,
        ...
        self.seed_value = parameters['parameters']['seed']
        ...
        # self.seed_value = parameters['parameters']['seed']Parameters.seed
        tf.random.set_seed(self.seed_value)
    ...
    def call(self, inputs):
        ...
        if tf.math.reduce_any(tf.math.is_nan(cons_features)):
            print("*" * 100)
            print("cons_features :", cons_features)
            print(inputs[0])
            print("*" * 100)
            assert False
        vars_features = self.var_embedding(vars_features)

        if tf.math.reduce_any(tf.math.is_nan(vars_features)):
            print("*" * 100)
            print("vars_features :", vars_features)
            print(inputs[2])
            print("*" * 100)
            assert False
        ...
        for variable in output_vars:
            if tf.math.reduce_any(tf.math.is_nan(variable)):
                print("*" * 100)
                print("checking  variable has NaN:", variable)
                print("*" * 100)
    ...
    def save_state(self, path):
        ...
        with open(path, 'wb') as f:
            for v_name in self.variables_topological_order:
                ...
                # print("v: " , len(v))
                saving_weights[v_name] = v.numpy()
            pickle.dump(saving_weights, f)
        # print("Saving!")
        # print("*"*100)
        # self.save_weigths(path)
    ...
    def restore_state(self, path):
        ...
        with open(path, 'rb') as f:
            saved_weight = pickle.load(f)
            for v_name in self.variables_topological_order:
                v = [v for v in self.variables if v.name == v_name][0]
                # self.variables[v_name] = saved_weight[v_name]
                v.assign(saved_weight[v_name])
            ...
            if train:
                with tf.GradientTape() as tape:
                    ...
                    if tf.math.reduce_any(tf.math.is_nan(logits)):
                        ...
                        self.save_state(path='./just_before_NaN.pickle')
                        # data, labels, totals_0
                        with open('input_tuple.dill', 'wb') as f:
                            dill.dump(input_tuple, f)
                        assert False
                    ...
                    loss = (loss * label.shape[0]) / (total_0[
                                                          0] + 100)  ## this is a quick fix, as there are far less action nodes compared to
                ...
                if tf.math.reduce_any(tf.math.is_nan(loss)):
                    print("*" * 100)
                    print("infinite loss!!", loss)
                    print("logits :", logits)
                    print("label  : ", labels[batches_counter])
